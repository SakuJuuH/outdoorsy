...
class ActivityRepositoryImpl @Inject constructor(private val assistantService: AiAssistantApiService) :
    AiAssistantApiService {
    override suspend fun startAiSession(
        request: AiAssistantRequestDto
    ): AiAssistantResponseDto {
        return assistantService.startAiSession(request)
    }
}